	...
	for _, testCase := range testCases {
		t.Run(testCase.Description, func(t *testing.T) {
			...
			if testCase.LoggerConfiguration.EnableFile {
				tempDir := os.TempDir() + "/TestLoggingAfterInitialized"
				if len(tempDir) < 2 && !strings.Contains(tempDir, "/TestLoggingAfterInitialized") {
					t.Errorf("Expected temp dir, but got... %s", tempDir)
				}
